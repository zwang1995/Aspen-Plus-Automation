...
# (v2) Updated at 1 Oct 2024 by Zihao Wang, zwang@mpi-magdeburg.mpg.de


import itertools
from aspen_utils_v2 import *
...
def run_Simulation(combine_loop, txt_file):
    ...
    # load Aspen Plus file in .bkp or .apw
    Aspen_Plus = Aspen_Plus_Interface()
    # Aspen_Plus.load_file(r"../simulation/ExtractiveDistillation.bkp", 0, 1)
    Aspen_Plus.load_file(r"../simulation/ExtractiveDistillation.apw", 0, 1)
    time.sleep(2)
    ...
    for run_Index, Input in enumerate(combine_loop):
        ...
        Aspen_Plus.Application.Tree.FindNode(r"\Data\Blocks\ED\Input\FEED_STAGE\FEED").Value = np.ceil(
            0.5 * NStage)  # the mixture is fed in the middle of the column
        ...
        hasERROR = Aspen_Plus.Application.Tree.FindNode(
            r"\Data\Results Summary\Run-Status\Output\UOSSTAT").Value  # for the entire flowsheet
    ...
    Aspen_Plus.close_file()


def main():
    # generate a set of operating configurations
    NStage_list = SequenceWithEndPoint(10, 100, 10)  # the number of stage of the column
    ...
    combine_loop = np.array(list(itertools.product(NStage_list, RR_list, StoF_list)))
